...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', async ({ roomId, username }) => {
    ...
    if (room.players.length === room.maxPlayers) {
      ...
      io.to(roomId).emit('game_start', {
        message: `🎉 成员满员！【${room.maxPlayers}人模式】由【${firstPlayer.username}】先手出牌！`,
        room,
  ...
  // 【核心修复】：有人打出新牌时，归零 Pass 计数器，强行更新桌面领头者
  socket.on('play_cards', ({ roomId, handInfo }) => {
    ...
    // 重置 pass 计数，并将领头牌主设为当前玩家 Socket ID
    room.passCount = 0;
    room.lastPlayedHand = {
      playerId: socket.id,
  ...
  // 【核心修复】：精确定位 Pass，只有连续 Pass 到达 maxPlayers - 1 次才判定全员 Pass 清空桌面
  socket.on('pass_turn', ({ roomId }) => {
    ...
    // 只有当连续 Pass 人数达到除打出最大牌以外的所有人（maxPlayers - 1）时，才触发清空桌面！
    ...
    io.to(roomId).emit('turn_changed', {
      currentTurnSocketId: nextPlayer.id,
      username: nextPlayer.username,
      lastPlayedHand: room.lastPlayedHand // 若桌面被清空，此处传 null
    });
  });
  ...
  socket.on('game_over', async ({ roomId, winnerUsername }) => {
    ...
    try {
      await User.findOneAndUpdate({ username: winnerUsername }, { $inc: { coins: 300 } }, { new: true });
      io.to(roomId).emit('game_result', {
        winner: winnerUsername,
        reward: 300,
        message: `🏆 恭喜玩家 【${winnerUsername}】 获得胜利！金币 +300！`
      });
      delete rooms[roomId];
